...
fb_api_http_chk(FbApi *api, SoupRequestHTTP *con)
{
    ...
    SoupMessage *res = soup_request_http_get_message(con);
                ...
                GAsyncReadyCallback callback)
{
    ...
    req = soup_session_request_http(priv->ses, "POST", url, NULL);
    ...
    printf("ooooo");
    SoupMessage *msg = soup_request_http_get_message(req);
    printf("eeee");
    if (priv->token != NULL) {
        data = g_strdup_printf("OAuth %s", priv->token);
        soup_message_headers_append(msg->request_headers, "Authorization", data);
        g_free(data);
    }

    soup_message_headers_append(msg->request_headers, "User-Agent", FB_API_AGENT);
    soup_message_headers_append(msg->request_headers, "Content-Type", "application/x-www-form-urlencoded; charset=utf-8");
...
FbApi *
fb_api_new(GSocketClient *gc, SoupSession *ses)
{
    FbApi *api;
    FbApiPrivate *priv;
    ...
    g_socket_client_set_tls(gc, TRUE);

    priv->gc = g_object_ref(gc);
    priv->ses = g_object_ref(ses);
...
fb_api_contact(FbApi *api, FbId uid)
{
    ...
    fb_json_bldr_add_str(bldr, "2", "100");
...
fb_api_cb_contacts_nodes(FbApi *api, JsonNode *root, GSList *users)
{
	...
	while (fb_json_values_update(values, &err)) {
		...
		user->name = fb_json_values_next_str_dup(values, NULL);
		user->icon = fb_json_values_next_str_dup(values, NULL);
        user->is_friend = g_strcmp0(str, "ARE_FRIENDS") || uid == priv->uid;
...
fb_api_contacts_after(FbApi *api, const gchar *cursor)
{
    ...
    fb_json_bldr_add_str(bldr, "3", "100");
                    ...
                    GError **error)
{
    ...
    thrd->is_group = fb_json_values_next_bool(values, FALSE);
    ...
    values = fb_json_values_new(root);
    fb_json_values_add(values, FB_JSON_TYPE_STR, FALSE,
                       "$.messaging_actor_id_only.id");
    fb_json_values_set_array(values, FALSE, "$.all_participant_ids.nodes");

    while (fb_json_values_update(values, &err)) {
        str = fb_json_values_next_str(values, "0");
        uid = FB_ID_FROM_STR(str);

        if (uid != 0 && uid != priv->uid) {
            FbId *copy = g_new (FbId, 1);
            *copy = uid;
            thrd->users = g_slist_prepend(thrd->users, copy);
        }
    }
                      ...
                      gpointer data)
{
    ...
    if (node == NULL) {
        fb_api_error(api, FB_API_ERROR_GENERAL,
                     _("Failed to obtain thread information: node not found"));
        json_node_free(root);
        return;
    }
...
fb_api_thread_create(FbApi *api, GSList *uids, const gchar *name)
{
    ...
    if (name != NULL && strlen(name) != 0) {
        fb_http_params_set_str(prms, "name", name);
    }
...
void
fb_api_threads(FbApi *api, gint count)
{
    JsonBuilder *bldr;

    bldr = fb_json_bldr_new(JSON_NODE_OBJECT);
    fb_json_bldr_add_strf(bldr, "1", "%d", count);
    fb_json_bldr_add_str(bldr, "2", "true");
    fb_json_bldr_add_str(bldr, "12", "true");
    fb_json_bldr_add_str(bldr, "13", "false");
...
FbApiThread *
fb_api_thread_dup_deep(const FbApiThread *thrd)
{
    return fb_api_thread_dup(thrd, TRUE);
}

GType
fb_api_thread_get_type (void)
{
...
fb_api_thread_dup(const FbApiThread *thrd, gboolean deep)
{
    ...
    if (deep) {
        ...
        for (l = thrd->users; l != NULL; l = l->next) {
            user = g_memdup(l->data, sizeof(FbId));
            ret->users = g_slist_prepend(ret->users, user);
        }

        ret->topic = g_strdup(thrd->topic);
        ret->last_message = g_strdup(thrd->last_message);
...
fb_api_thread_reset(FbApiThread *thrd, gboolean deep)
{
    ...
    if (deep) {
        g_slist_free_full(thrd->users, (GDestroyNotify) g_free);
        g_free(thrd->topic);
        g_free(thrd->last_message);
    }
...
FbApiUser *
fb_api_user_dup_deep(const FbApiUser *user)
{
    return fb_api_user_dup(user, TRUE);
}

GType
fb_api_user_get_type (void)
{
    static GType type_id = 0;
